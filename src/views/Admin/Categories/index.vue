<template>
  <div>
      ...
      <div class="container" v-else>
        <div class="row">
          <div class="col-md-4 col-sm-6 col-xs-12" v-for="category in categories" :key="category.id">
            <category-card :category="category" @deleteCategory="showDeleteModalHandler" @editCategory="showModalHandler"></category-card>
          </div>
        </div>
      </div>
    </div>
    ...
    <delete-modal :show="showDeleteModal" @close="hideModalHandler" :loading="deleteLoading" @deleteCategory="deleteCategory"></delete-modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    deleteCategory() {
      ...
      this.$store.dispatch("AdminStore/deleteCategory", this.selectedCategoryToDelete).then(() => {
